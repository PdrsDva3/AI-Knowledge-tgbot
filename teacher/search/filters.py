...
from db.db_teacher import get_filter_students, check_id
...
async def print_teacherf(callback: CallbackQuery, state: FSMContext):
    ...
    student_list = data.get("list", [])
    index = data.get("index", 0)

    if index < len(student_list):
        student = student_list[index]
        await callback.message.edit_text(
            text=TEACHER_DATAf.format(student["name"], student["grade"], student["sphere"], student["bio"]),
...
async def get_random_studentsf(grade, sphere, student_id) -> list[dict]:
    list_ = await get_filter_students(grade, sphere, student_id)
...
async def searching(callback: CallbackQuery, state: FSMContext):
    ...
    gr = student_data["grade"]
    sp = student_data["sphere"]
    if "list" not in student_data:
        random_list = await get_random_studentsf(gr, sp, callback.from_user.id)
...
async def agree_request(callback: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(student_id, STUDENT_DATA.format(teacher.name, teacher.grade,
...
async def deny_request(callback: CallbackQuery):
    ...
    student_info = (await db.db_student.get_all(student_id))[0]
